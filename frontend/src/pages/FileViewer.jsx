...
function FileViewer() {
        ...
        ws.onmessage = (event) => {
          try {
            ...
            if (data.type === 'review_cycle_update' && data.review_cycle_id === reviewCycleId) {
              console.log('🔄 Review cycle status updated:', data.status)
  ...
  const trackView = async () => {
    try {
      ...
      console.log('👤 User is member:', response.data.user_is_member)
      
      setViewTracked(true)
      
      // If status changed, show immediate feedback
      ...
      if (error.response?.status === 403) {
        toast.error('You do not have permission to view this proof')
      } else {
  ...
  const fetchAssetData = async () => {
    try {
      ...
      if (response.data?.review_cycles && response.data.review_cycles.length > 0) {
        ...
        // No review cycle exists, create one automatically
        console.log('📋 No review cycle found, creating one automatically...')
        await createReviewCycleForAsset(response.data.id)
      }
  ...
  const createReviewCycleForAsset = async (assetId) => {
    try {
      ...
      const response = await api.post('/workflows/review-cycles/auto_create/', {
        asset_id: assetId
      })
      
      console.log('✅ Review cycle created:', response.data)
      
      if (response.data.review_cycle) {
        setReviewCycleId(response.data.review_cycle.id)
        console.log('📋 Set review cycle ID:', response.data.review_cycle.id)
        
        // Show success message
        toast.success('Review cycle created automatically')
        
        // Refresh asset data to get the updated review cycle
      ...
      // Don't show error to user unless it's critical, as this is background functionality
