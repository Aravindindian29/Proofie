...
from .models import ApprovalGroup, GroupMember, ReviewCycle, Activity
...
class WorkflowService:
    ...
    def record_member_decision(member, decision, feedback=''):
        ...
        decision_text = decision_display_map.get(decision, decision.replace('_', ' ').title())

        

        content = f"{decision_text} the proof"

        if feedback:

            content += f" with feedback: {feedback}"

        

        activity = Activity.objects.create(

            review_cycle=member.group.review_cycle,
        ...
        WorkflowService.broadcast_activity(activity)
    ...
    def broadcast_activity(activity):
        ...
        Broadcast new activity via WebSocket to all relevant users.
        ...
        try:
            ...
            for user in users_to_notify:
                ...
                try:
                    ...
                    logger.error(f"Failed to send activity to user {user.id}: {e}")

            

            logger.info(f"Broadcasted activity {activity.id} to {success_count}/{len(users_to_notify)} users")
